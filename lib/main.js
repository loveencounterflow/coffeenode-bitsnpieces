...
(function() {
  var LODASH, njs_fs, njs_path, permute, rpr, validate_isa_number,
    __slice = [].slice;
  ...
  this.shuffle = function(list, ratio) {
    ...
    while (true) {
      ...
      if (ratio >= 1 || this.random_number() <= ratio) {
        that_idx = this.random_integer(0, this_idx);
  ...
  this.caller_description_from_locator = function(locator) {
    var R, col_nr, col_nr_txt, line_nr, line_nr_txt, match, name, route, _;
    match = locator.match(/^(.+)\/([^\/]+)#([0-9]+):([0-9]+)$/);
    if (match == null) {
      throw new Error("illegal stack locator " + (rpr(locator)));
    }
    _ = match[0], route = match[1], name = match[2], line_nr_txt = match[3], col_nr_txt = match[4];
    ...
    R['source'] = this._source_line_from_description(R);
    return R;
  };

  this._source_line_from_description = function(description) {
    var R, line_nr, route, source_lines;
